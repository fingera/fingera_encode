...
static void encdec_test(const std::string& hex, const std::string& encoded) {
  ...
  EXPECT_EQ(hex_decode(hex.c_str(), hex.size(), data.data()), data.size());
  char out_char[4096];
  EXPECT_TRUE(base58_encode_max_len(data.size()) < sizeof(out_char));
  if (base58_encode_max_len(data.size()) < sizeof(out_char)) {
    size_t out_size = base58_encode(data.data(), data.size(), out_char);
    ...
    EXPECT_TRUE(base58_decode_max_len(out_size) < sizeof(out_data));
    if (base58_decode_max_len(out_size) < sizeof(out_data)) {
      size_t data_size = base58_decode(out_char, out_size, out_data);
...
TEST(base58, custom) {
  ...
  EXPECT_EQ(base58_encode(buffer, sizeof(buffer), out_char), result.size());
  EXPECT_EQ(result, out_char);
  size_t out_char_size = base58_decode(result.c_str(), result.size(), out_char);
  EXPECT_EQ(out_char_size, sizeof(buffer));
  EXPECT_ZERO(memcmp(out_char, buffer, sizeof(buffer)));
  out_char_size = base58_decode(
