...
static size_t _to_base32(const void *buf, size_t buf_size, char *out,
                         int is_raw) {
...
static size_t _from_base32(const char *str, size_t str_len, void *buf,
                           int is_raw) {
  size_t tail = str_len % 8;
  size_t blocks = str_len / 8;
...
size_t base32_encode_len(size_t buf_size) { return ((buf_size + 4) / 5) * 8; }
size_t base32_decode_max_len(size_t str_len) { return ((str_len + 7) / 8) * 5; }

void base32_encode(const void *buf, size_t buf_size, char *str) {
  _to_base32(buf, buf_size, str, 0);
}
size_t base32_decode(const char *str, size_t str_len, void *buf) {
  ...
  return _from_base32(str, str_len, buf, 0);
}
...
size_t base32raw_decode_max_len(size_t str_len) {
  size_t size = (str_len / 8) * 5;
  return size + TAIL_TO_BYTES[str_len % 8];
}

size_t base32raw_encode(const void *buf, size_t buf_size, void *str) {
  return _to_base32(buf, buf_size, (char *)str, 1);
}

size_t base32raw_decode(const void *str, size_t str_len, void *buf) {
  return _from_base32((const char *)str, str_len, buf, 1);
}
