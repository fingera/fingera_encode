...
size_t hex_encode_len(size_t buf_size) { return buf_size * 2; }
size_t hex_decode_max_len(size_t str_len) { return str_len / 2; }
...
void hex_reverse_encode(const void *buf, size_t buf_size, char *str,
                        int upper) {
  unsigned char byte;
  unsigned char *bytes = (unsigned char *)buf;
  const char *hex_str = upper ? HEX_STRING_UPPER : HEX_STRING_LOWER;
