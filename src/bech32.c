...
bool bech32_decode(const char *str, size_t str_len, char *hrp, size_t *hrp_size,
  ...
  if (is_illegal(str, str_len)) return false;
  ...
  if (*hrp_size < (size_t)(pos - str)) return false;
  *hrp_size = pos - str;
  ...
  const char *end = (str + (str_len - 6));
  if (*b32_size < (size_t)(end - pos)) return false;
  *b32_size = (size_t)(end - pos);
  while (pos != end) {
  ...
  if (!verify_checksum(hrp, *hrp_size, (uint8_t *)b32, *b32_size, checksum))
    return false;
  return true;
}